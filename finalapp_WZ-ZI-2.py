...
def write_temp_upload_crops(uploaded, suffix: str):
...
CROP_SPLIT = re.compile(r'[A-Z]{2}_[0-9]+\.[0-9]')

def parse_id_rich_all(header: str, default_label: str = ""):
    """
    ...
    if len(parts) == 5:
        geneinfo, geneinfo_id, dbname, acc, protein_species = parts[0].strip(), parts[1].strip(), parts[2].strip(), parts[3].strip(), parts[4].strip()
        protein_species_parts = protein_species.split('[')
    else:
        acc = raw[1:] if raw.startswith(">") else raw

    if len(protein_species_parts) > 1:
        protein = protein_species_parts[0].strip()
        species = protein_species_parts[1].replace('[','').replace(']','').strip()
    else:
        protein = "nope"
        species = "nope"
    
    return acc, species, protein

def parse_id_rich_crop(header: str, default_label: str = ""):
    """
    Try to parse accession, species, and name/gene from diverse FASTA headers.
    Expected best case: Accession|Species|GeneOrDesc
    Fallbacks:
    ...
    # Try pipe-separated first
    match = CROP_SPLIT.search(header)
...
def render_results(out_path: str, crop_label: str, crop_is_protein: bool, pident_min: int):
    ...
    crop_parsed = df["qseqid"].apply(lambda s: parse_id_rich_crop(str(s), default_label=crop_label))
    all_parsed  = df["sseqid"].apply(lambda s: parse_id_rich_all(str(s), default_label="Allergen"))
...
with tab_p:
    ...
    if st.button("Run BLASTP"):
        ...
        try:
            q_faa = write_temp_upload_crops(crop_upload, ".faa") if crop_upload else write_temp_text(crop_text, ".faa")
            a_faa = write_temp_upload_crops(all_upload, ".faa") if all_upload else write_temp_text(all_text, ".faa")
